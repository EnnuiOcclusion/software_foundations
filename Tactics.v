...
It looks like apply is Modus Ponens. (The way that affirms by affirming).

(1) (P implies Q) and (2) (P) are asserted to be true, and therefore Q must be true.

Find the left portion of the given in your subgoal. Replace it with the right side of the given fact via rewrite (->).
Apply reflexivity.

Works in practice of reflexivity and rewriting for an entire subgoal with an entire fact or implication.
...
Apply is a rewrite with symmetry. In practice this amounts to matching an entire subgoal with the implication from a hypotheses. It's a higher ordder reflexivity with implied rewriting.
