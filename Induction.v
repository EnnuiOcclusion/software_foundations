...
Proof.
  ...
  - simpl. induction n as [| n' IHn' ].
    + simpl. reflexivity.
    + simpl. rewrite IHn'. rewrite negb_involutive. reflexivity. Qed.
(** [] *)
...
Lemma mult_ident_l : forall n : nat,
    n * 1 = n.
Proof.
  induction n as [| n' IHn'].
  - simpl. reflexivity.
...
Lemma mult_dist_plus_l : forall n m p : nat,
    n * (m + p) = (n * m) + (n * p).
Proof.
  intros n m p.
  induction n as [| n' IHn'].
  ...
  - simpl.
    ...
    rewrite plus_swap.
    rewrite <- plus_assoc.
    replace (n' * m + m) with (m + n' * m).
    rewrite plus_swap.
    rewrite plus_assoc.
    reflexivity.
    + rewrite plus_comm. reflexivity. Qed.

Lemma mult_dist_plus_r : forall n m p :nat,
    (n + m) * p = (n * p) + (m * p).
Proof.
  intros n m p.
  induction n as [| n' IHn'].
...
Proof.
  ...
  - simpl.
    ...
    reflexivity.
  - rewrite plus_1_S.
    rewrite mult_dist_plus_l.
    rewrite mult_ident_l.
    rewrite mult_dist_plus_r.
    rewrite <- mult_ident_l.
    simpl.
    rewrite <- plus_n_O.
    rewrite mult_ident_l.
...
Proof.
  intros n m p.
  rewrite mult_dist_plus_r. reflexivity. Qed.
...
Proof.
  ...
  induction n as [| n' IHn'].
  - simpl. reflexivity.
  - simpl. rewrite IHn'. rewrite mult_dist_plus_r. reflexivity. Qed.
...
Theorem nat_bin_nat_iso : forall n : nat,
...
Theorem bin_nat_bin_iso : forall b : bin,
...
Fixpoint normalize (b : bin) : bin :=
  match b with
  | Zero => Zero
  | Even b => nat_to_bin (S (S (bin_to_nat b)))
  | Odd b => nat_to_bin (S (bin_to_nat b))
  end.

Theorem prove_normalize : forall b : bin,
