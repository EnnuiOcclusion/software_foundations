...
Theorem bag_count_0_sum: forall (s1 s2 : bag),
...
Theorem rev_injuctive_backwards : forall (l1 l2 : natlist),
    l1 = l2 -> rev l1 = rev l2.
Proof.
  intros l1 l2 h.
  induction l1 as [| n1' l1' IHl1'].
...
Theorem rev_injuctive : forall (l1 l2 : natlist),
    rev l1 = rev l2 -> l1 = l2.
Proof.
  intros l1 l2 h.
...
Proof.
  ...
  - simpl.
    reflexivity.
  - simpl.
    assert (H: beq_nat n n = beq_id (Id n) (Id n)).
    {
      reflexivity.
    }
    rewrite H.
...
Three.
Baz1 is the unit of the type, so it has only 1 element.
Baz2 wraps a boolean in a baz, so we have two additional elements, one for each boolean.

On second look none! You have two ways of getting a Baz given a Baz, but there's no unit type to get your inital baz on.
