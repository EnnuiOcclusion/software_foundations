...
combine : forall X Y, list X -> list Y -> list (X*Y)
...
Fixpoint splitX {X Y : Type} (l : list (X*Y)) : list X :=
  match l with
  | nil => nil
  | (x,y) :: t => x :: splitX t
  end.

Fixpoint splitY {X Y : Type} (l : list (X*Y)) : list Y :=
  match l with
  | nil => nil
  | (x,y) :: t => y :: splitY t
  end.
...
Definition filter_even_gt7 (l : list nat) : list nat
  := filter (fun l => (evenb l) && (blt_nat 7 l)) l.
...
(** If I wanted to fold over a hashmap and extract values away from keys.
    Any time I'm working with monads and want to extract value from context. *)
(** [] *)
...
(* Holy Shit!!! -> is the binary operator on types!!!! *)
(* What is the unary operator? Is it id? *)
...
Proof.
  intros X Y Z f x y.
  assert (h: prod_curry (prod_uncurry f) = f).
  {
    reflexivity.
  }
  rewrite h.
...
Proof.
  intros X Y Z f [].
  assert (h : prod_curry f = (fun x y => f (x,y))).
  {
    simpl.
    reflexivity.
  }
...
You out of bounds son! Keep it in between the lines.
*)
(** [] *)
